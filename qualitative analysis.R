...
db <- import (here ('data/c480dd10-7d8b-46a6-8f36-d557313f4168.xlsx'))

glimpse (db)
db$is_epix <- ifelse (db$Modalidade=='ESPECIAL', 1, 0)

colnames (db) <- c('autor', 'nemenda', 'ano', 'nconv', 'acesso', 'sitconv',
                   'obj', 'modalidade', 'cnpj', 'nomeprop', 'siguf', 'nomemun',
                   'banco', 'rp', 'valorcusteio_epix', 'valorinvest_epix', 
                   'total', 'empenhado', 'liberado', 'relatorio', 'ppol', 
                   'prog', 'is_epix')
